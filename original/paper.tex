...
\begin{itemize}
  ...
  \begin{itemize}
    \item \textbf{{$N_{windows}=74{,}771$}}: number of non-overlapped 10-seconds signal windows analyzed along the full recording period.
...
The objective functions in the baseline XGBoost models are binary cross-entropy for classification and mean squared error for regression. We used gradient boosted trees, a learning rate $\eta = 0.05$, a maximum tree depth of 10, with 500 boosting rounds.
...
\begin{itemize}
  ...
  \item For the regression task (vessel distance estimation): Global Mean Absolute Error (MAE) defined considering all the available vessels in the dataset, and $1{,}000\,m$ MAE, defined for vessel distances below $1{,}000\,m$ (to assess precision in vessel nearby positions). The lower the MAE values, the better the performance.
\end{itemize}

Table~\ref{tab:performance} shows the performance metrics corresponding to $N_C \in \lbrace 10, 50, 250 \rbrace$ channels. To summarize, the XGBoost classifier achieved a global $F_1$-score exceeding 89\% for vessel detection, with reasonably balanced class-wise $F_1$ metrics, despite the clear data unbalance issues (see Fig.~\ref{fig:data-balance}). Regression models estimated global vessel distance with a MAE in the order of $800\,m$, which is again reasonably considering the unrestricted conditions, that include vessels as far as $8\,km$ from the cable. The $1{,}000\,m$ MAE gets as low as $171\,m$, which is also a good precision for for vessels that are in a $1{,}000\,m$ radius from the cable. Finally, all the performance results improve when using larger spatial contexts.

\input{table-performance.tex}

The good results achieved with our initial experimental approach suggests that the provided data do contain discriminative features for the proposed tasks, being a strong evidence of the dataset's technical validity. We carried out a much more detailed experimental work on this dataset, that is fully described in~\cite{ramirez2025vessel}.
...
E.E.R.-T., S.E.P.-C. J.M.-G., D.P.-P. and J.T. carried out the dataset design, designed the experimental study and and performed data processing and model implementation tasks. R.V. coordinated access to the DAS data, metadata and preprocessing scripts. S.M.-L. and M.G.-H. supervised optical sensing and signal interpretation. All authors reviewed and approved the manuscript.
