...
The dataset contains over $74,000$ preprocessed and labeled data frames from more than ten days of continuous recording along a $28\,km$ buried fiber segment in the southern North Sea. Each frame includes 100-band spectral energy features and anonymized vessel metadata from the Automatic Identification System (AIS). The dataset, processing workflow, and example code together provide a benchmark for developing and validating DAS-based methods for vessel monitoring and submarine cable protection.
\end{abstract}
...
Figs. XXX and YYY show two examples of the generated strain signals for scenarios with a vessel nearby or in the absence of nearby vessels, respectively.
...
The AIS data provided by the cable operator included vessel identifiers (MMSI), timestamps, coordinates, course, and speed for 745 unique vessels with 64,417 position reports. A relevant problem when using AIS data is the unpredictable and typically low position update rates~\cite{emmens2021promisesAIS}. A detailed analysis on our DAS data showed that $88\%$ of vessels reported positions only every 1 to 3 minutes, which may imply a significant positional uncertainty. To overcome this issue, we decide to apply a linear interpolation between consecutive reported positions at $1\,Hz$. %, discarding trajectories with gaps exceeding $60$ minutes.
We did not have the possibility of a direct quantitative validation of this simple interpolation approach (provided AIS data was the only available source of vessel positioning), but the good results obtained (see Section~\ref{sec:technical validation}) served us a kind of ``indirect validation'', so that we did not require to apply more sophisticated interpolation methods~\cite{guo2021improved}.

After the interpolation process, we had over $1,200,000$ AIS position entries, which we further enriched with additional AIS metadata (namely, vessel type, length and beam)  that could be useful in further studies or AI/ML tasks requiring information related to vessel type and size.
...
\vspace{0.3cm}
\noindent\textbf{figure:} METER FIGURA CON NÚMERO DE EJEMPLOS DISPONIBLES PARA BARCOS POR DEBAJO DE DISTANCIAS VARIAS 500 1000 1500 2000 3000, ETC, PARA JUSTIFICAR VISUALMENTE LA SELECCIÓN DE LA ZONA DEL CANAL Y PLANTEAR OTRAS COMO LA MÁS CERCANA AL INTERROGADOR. METER TAMBIÉN FIGURA DE HISTOGRAMA DE MÍNIMAS DISTANCIAS DE BARCOS A CABLE, SI QUEDA BIEN.
...
ERICK, PLEASE METE LAS ECUACIONES DE CÁLCULO EXACTO DE LA ENERGÍA EN BANDAS DEFININIENDO LOS SUMATORIOS QUE TOCAN Y LOS VALORES DE AMPLITUD (MÓDULO) DE LA FFT PARA REFERENCIA DE LOS INTERESADOS
