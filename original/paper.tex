...
We carried out the initial technical validation by applying a XGBoost ML algorithm to the two machine learning tasks defined in Section~\ref{sec:ml-tasks}. The vessel detection task was set with a distance threshold of $1000\,m$, and the vessel distance estimation task evaluated two cases: unrestricted distance estimation, and distance estimation for vessels closer than $1000\,m$ to the cable.
...
Table~\ref{tab:performance} shows the performance metrics corresponding to $N_C \in \lbrace 10, 50, 250 \rbrace$ channels. To summarize, the XGBoost classifier achieved an overall F1-score exceeding 89\% for vessel detection within $1000\,m$, with reasonably balanced global and class-wise $f_1$ metrics, despite the clear data unbalance issues (see Fig.~\ref{fig:data-balance}). Regression models estimated global vessel distance with a MAE of, and a $1000\,m$ MAE of 171~m. Results improved with larger spatial contexts.

\input{table-performance.tex}

The good results achieved with our initial experimental approach suggests that the provided data do contain discriminative features for the proposed tasks, being a strong evidence of the dataset's technical validity. We carried out a much more detailed experimental work on this dataset, described in~\cite{ramirez2025vessel}.
