...
The mathematical details for the feature extraction calculation follows. We first assume that $x_c^{(n)}[m]$ denote the discrete version of the $n^{th}$ windowed strain signal corresponding to channel $c$ starting at $n·T_\omega$ seconds, and that $X_c^{(n)}[k]$ is the $N_{\mathrm{FFT}}$-point FFT of $x_c^{(n)}[m]$, with $n=0, 1, \dots, N_{windows}-1$ and $c=0,1,\dots N_{channels}-1$ being $N_{windows}$ and $N_{channels}$ the number of available windows and available channels, respectively. Each spectral band $b = 0, 1, \dots, N_{bands}-1$ is defined by its FFT-bin limits $\big[k_b^{-},\, k_b^{+}\big]$, where $0 \le k_b^{-} \le k_b^{+} < N_{\mathrm{FFT}}$.
...
To simplify the dataset distribution all the data are stored in a HDF5 archive with the following structure (summarized in Table~\ref{tab:dataset-content}):

\begin{itemize}
  \item \texttt{X}: A 3D NumPy array of shape \texttt{($N_{windows}$, $N_{channels}$, $N_{bands}$)}, containing the feature vectors.
  \begin{itemize}
    \item \textbf{\texttt{$N_{windows}$=74771}}: number of non-overlapped 10-seconds signal windows analyzed along the full recording period.
    \item \textbf{\texttt{$N_{channels}$=250}}: number of spatial channels (sensor positions along the selected fiber segment).
    \item \textbf{\texttt{$N_{bands}$=100}}: number of energy-band features per channel.
  \end{itemize}

  \item \texttt{y}: A 1D NumPy array of shape \texttt{($N_{windows}$,)} containing the distance (in meters) to the closest vessel during each 10-second window. This continuous variable supports regression tasks directly, or can be converted into classification labels by applying user-defined distance thresholds.

  \item \texttt{datetimes}: An array of strings in the HDF5 file that records the UTC timestamp (ISO-8601 format) corresponding to each 10-second window, with shape \texttt{($N_{windows}$,)}.

  \item \texttt{ship\_info}: A group in the HDF5 containing AIS metadata (vessel type, length, beam) of the vessel used to generate the \texttt{y} label.
\end{itemize}
...
\todojmg{Erick, check dimensiones please. ship_info tiene tamaño fijo?... entiendo que también 74771, ¿no?...}
\begin{table}[H]
  \centering
  ...
  \texttt{ship\_info} & AIS vessel metadata (type, length, beam) used to generate \texttt{y} labels & --- \\
