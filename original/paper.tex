...
AQUÍ REFERENCIAR A LA ARQUITECTURA DE DATA CURATION QUE HEMOS HECHO, Y REFLEJAR LOS MISMOS NOMBRES QUE EN LA FIGURA.
...
\subsection{Geospatial Tagging: Geographical+Bathymetri Data Processing}
...
\subsection{AIS Data Processing}
\label{sec:ais-data-processing}

\todojmg{Erick, revisa las estadísticas de AIS que pone aquí.}
Raw AIS data provided by the cable operator included vessel identifiers (MMSI), timestamps, coordinates, course, and speed for 745 unique vessels (64,417 reports). A relevant problem when using AIS data is the unpredictable and typically low position update rates~\cite{emmens2021promisesAIS}. A detailed analysis on our DAS data showed that $88\%$ of vessels reported positions only every 1 to 3 minutes, which may imply a significant positional uncertainty. To overcome this issue, we decide to apply a linear interpolation between consecutive reported positions at $1\,Hz$, discarding trajectories with gaps exceeding $60$ minutes. We did not have the possibility of a direct quantitative validation of this simple interpolation approach (provided AIS data was the only available source of vessel positioning), but the good results obtained (see Section~\ref{sec:technical validation}) served us a kind of ``indirect validation'', so that we did not require to apply more sophisticated interpolation methods~\cite{guo2021improved}.

After the interpolation process, we had over $1,200,000$ AIS positios entries, which we further enriched with additional AIS metadata (namely, vessel type, length and beam)  that could be useful in further studies or AI/ML tasks requiring information related to vessel type and size.


\subsection{ML Tasks and Data Balance Considerations}
\label{sec:ml-tasks}

The \texttt{Marlinks-NS DAS} dataset has been designed to support two complementary machine learning (ML) tasks related to vessel monitoring using submarine fiber optic cables. The first task, \emph{vessel detection}, is formulated as a binary classification problem that is oriented to discriminate whether there is a vessel closer than a given distance threshold to the cable. The second task, \emph{distance estimation}, is defined as a regression problem in which the model aims to predict the distance between the cable and the nearest vessel. In both cases the required labeling information is derived from the synchronized Automatic Identification System (AIS) data.

To ensure both tasks are representative and statistically balanced, a detailed analysis of the vessel geographical distribution  along the monitored cable was performed. Initial inspection of the ten-day continuous recording revealed strong spatial nonuniformity in vessel passages—mainly concentrated in the section of the cable intersecting the main navigation corridor. Regions near the cable ends presented very low vessel activity, leading to class imbalance and data sparsity that would bias ML model training and evaluation. To mitigate this, a contiguous portion of $2553\,m$ was selected, to corresponding to a subset of $N_{channels}=250$ equally spaced spatial channels located between $14707\,m$ and $17260\,m$ from the interrogator. This segment exhibits a balanced distribution of vessel distances, offering sufficient representation of both “vessel-present” and “no-vessel” cases, as well as a broad dynamic range of distances suitable for regression.

For the classification task, the dataset can be further partitioned according to distance thresholds (e.g., 0–500 m, 500–1000 m, 1000–1500 m, etc.), enabling flexible problem formulations with controllable class ratios. For regression, the continuous distance label allows quantitative modeling of vessel proximity with high temporal resolution.
...
FALTA MUCHO DETALLE, CONTAR 10 SEGUNDOS, ENVENTANADO, ETC.
...
The spectral range was split into $N_{bands}=100$
The \href{https://github.com/UAH-PSI/das-vessel-detection/blob/main/data/fbands.csv}{the GitHub repository}
